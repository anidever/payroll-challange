...
def upgrade():
    ...
    op.create_index(op.f('ix_time_report_date'), 'time_report', ['date'], unique=False)
    op.create_index(op.f('ix_time_report_employee_id'), 'time_report', ['employee_id'], unique=False)
    op.create_index(op.f('ix_time_report_report_id'), 'time_report', ['report_id'], unique=False)
    job_group = postgresql.ENUM('A', 'B', name='job_group')
    job_group.create(op.get_bind())
    op.add_column('time_report', sa.Column('job_group', sa.Enum('A', 'B', name='job_group'), nullable=False))
